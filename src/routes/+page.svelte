<script lang="ts">
	...
	if (browser) {
		let camera, scene, renderer, controls, updateCompute, helpersMaterial
		let pointerMoveTimer
		const INACTIVITY_DELAY = 5000 // milliseconds
		const attractors = [] // needs to be global for visibility upon mouse activity
		...
		function init() {
			...
			helpersMaterial = new THREE.MeshBasicMaterial({ side: THREE.DoubleSide })
			...
			function hideAttractors() {
				// grab all the attractors, traverse children for materials, call GSAP to fade in
				// and don't forget the helpers
				for (const attractor of attractors) {
					attractor.controls.enabled = false
					attractor.controls.detach()
					attractor.helper.visible = false
					helpersMaterial.visible = false
				}
			}
			...
			function showAttractors() {
				for (const attractor of attractors) {
					...
					attractor.helper.visible = true
					helpersMaterial.visible = true
				}
			}
			...
			gui
				.add({ controlsVisible: attractors[0].controls.visible }, 'controlsVisible')
				.onChange((value) => {
					for (const attractor of attractors) {
						if (value === true) {
							attractor.controls.attach(attractor.reference)
